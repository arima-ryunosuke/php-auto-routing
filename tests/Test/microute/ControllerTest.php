...
class ControllerTest extends \ryunosuke\Test\AbstractTestCase
{
    ...
    function test_dispatch_content_type_array()
    {
        $service = $this->provideService([
            'parameterContexts' => [
                'csv' => 'text/csv',
        ...
        /// context なし
        $request->attributes->set('context', null);
        $request->attributes->set('ctype', null);
        $controller = new DispatchController($service, 'context', $request);
        ...
        $request->attributes->set('ctype', 'hoge/fuga; piyo');
        $response = $controller->dispatch();
        $this->assertEquals('hoge/fuga; piyo', $response->headers->get('Content-Type'));

        /// context あり
        $request->attributes->set('context', 'csv');
        $request->attributes->set('ctype', null);
        $controller = new DispatchController($service, 'context', $request);
    ...
    function test_dispatch_content_type_callback()
    {
        $service = $this->provideService([
